...
##########################
# NBA_log_intensity_25_18_MLE_initial_6000iters <- readRDS("./NBA_log_intensity_25_18_MLE_initial_6000iters.rds")
##############################
# replicate_id <- 100
# cluster_num = 
# max(NBA_log_intensity_25_18_MLE_initial_6000iters[[replicate_id ]]$zout)
##############################
...
for(i in 1:cluster_num){
  # assign(paste0("M",i), NBA_log_intensity_25_18_MLE_initial_6000iters[[replicate_id]]$Mout[,,i] + log(2))
  assign(paste0("M",i,"_raw"), readRDS(paste0("./M", i, "_raw.rds") ) )
  # rho <- c(0.9,0.6,0.3)
...
###
V <- noise_factor * ar1_cor(ncol(M1), rho_factor)
###
sim_25by18 <- foreach(j = 1:num_replicates, .errorhandling = "pass")%dopar%{
  # 
  ...
  ARI_mvnormalmixEM_oracle <- NA
  #  
  ARI_mvnormalmixEM_MFM <- NA
...
time2 <- Sys.time()
saveRDS(sim_25by18,
        paste0("./simulation_results/sim_25by18_",setting_id,"_new.rds"))
#####################################