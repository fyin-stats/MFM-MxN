...
colnames(M1) <- 1:6
M1_long <- melt(M1)
p1 <- ggplot(M1_long, aes(x = Var1, y = Var2)) +
...
colnames(M2) <- 1:6
M2_long <- melt(M2)
p2 <- ggplot(M2_long, aes(x = Var1, y = Var2)) +
  geom_raster(aes(fill=value)) +
  scale_fill_gradient(low="white", high="red") +
  labs(x="X", y="Y", title=paste("")) +
  theme_bw() + theme(axis.text.x=element_text(size=9, angle=0, vjust=0.3),
...
colnames(M3) <- 1:6
M3_long <- melt(M3)
p3 <- ggplot(M3_long, aes(x = Var1, y = Var2)) +
  geom_raster(aes(fill=value)) +
  scale_fill_gradient(low="white", high="red") +
  labs(x="X", y="Y", title=paste("")) +
  theme_bw() + theme(axis.text.x=element_text(size=9, angle=0, vjust=0.3),
...
#####################################
saveRDS(sim_10by6,
        paste0("sim_10by6_",signal_strength,"_noise025_3clusters",".rds"))
#####################################
#####################################
## calculation of adjusted rand index
## probability of selecting the correct number of clusters
## estimation accuracy of mean matrices
## estimation accuracy of covariance matrices
#####################################